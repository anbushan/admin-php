  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result["error"] == true) {
        ...
        if (window.location.href.indexOf("admin/attributes/") > -1) {
          setTimeout(function () {
            location.reload();
          }, 600);
        }
...
$(document).on("click", ".save-settings", function (e) {
  ...
  if (
    ...
    if (simple_stock < 0) {
      iziToast.error({
        message:
          '<span style="text-transform:capitalize">Stock cant be less then 0 !</span> ',
      });
      location.reload();
      return false;
    }
...
$(document).on("click", ".update_status_rider", function (e) {
  ...
  //   var delivery_otp_system = $(".is_rider_otp_setting_on").val();
  //   console.log(delivery_otp_system);
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result["error"] == false) {
        ...
        //   message: result["message"]
        // });
        // if(result["result"]["is_rider_otp_setting_on"] == 1 &&);
...
//             is_open: 1
//           });
//           $('input[name="F"]').val(JSON.stringify(availTuesday));
//           success = false;
//         } else {
//           success = true;
//         }
...
//       iziToast.error({
//         message:
//           '<span style="text-transform:capitalize">Please, Select time for working day!</span> '
//       });
//     }
//     if (time_validation) {
//       iziToast.error({
//         message:
//           '<span style="text-transform:capitalize">Please, Close time can not laser then open time! Set proper time</span> '
//       });
//     }
//   } else {
//     iziToast.error({
//       message:
//         '<span style="text-transform:capitalize">Please, Select atleast one day for working!</span> '
//     });
//   }
// }
...
$("#save_add_ons").on("click", function (event) {
  ...
  if (addon_price < 0) {
    iziToast.error({
      message:
        '<span style="text-transform:capitalize">Price cant be less then 0 !</span> ',
    });
    return false;
  }
  if (addon_calories !== "" && addon_calories < 0) {
    iziToast.error({
      message:
        '<span style="text-transform:capitalize">Calories cant be less then 0 !</span> ',
    });
    return false;
  }
  var is_exist = false;
...
$("#sign_up_restro_form").on("submit", function (e) {
  e.preventDefault();
  var formdata = new FormData(this);
  formdata.append(csrfName, csrfHash);
...
$("#save_charges").on("click", function (event) {
  ...
  if (filter == "") {
    iziToast.error({
      message:
        '<span style="text-transform:capitalize">Please, YOu can not set blank for range wise delivery charge system.</span> ',
    });
    is_error = true;
    return false;
  }
  ...
  for (let i = 0; i < filter.length; i++) {
    if (
      ...
      iziToast.error({
        message:
          '<span style="text-transform:capitalize">Please, Set proper ranges and prices to setup the range wise charges.</span> ',
      });
      is_error = true;
      return false;
    }
  ...
  if (is_error == false) {
    ...
    if (typeof charges != "undefined" && charges !== null) {
      ...
      iziToast.success({
        message:
          '<span style="text-transform:capitalize">Charges Saved</span> ',
      });
      $("#submit_btn").html("Submit").attr("disabled", false);
    } else {
      iziToast.error({
        message:
          '<span style="text-transform:capitalize">Please, Enter Proper Range for Charges !</span> ',
      });
    }
  }
});
...
window.actionEvents = {
  "click .edit-city": function (e, value, row, index) {
    // console.log(row.city_names[0].name);
    ...
    // var bordering_city_ids = $("#bordering_city_ids").val();
...
/** table booking  */
// $(".table_cart").on("click", function () {
//   var elements = document.getElementsByClassName("table_cart");
//   for (var i = 0; i < elements.length; i++) {
//     var element = elements[i];
//     // Check if the class value matches the desired class
//     if (element.classList.contains("table_cart")) {
//     // Get the value of the id attribute
//       var idValue = element.id;
//       console.log(idValue);
//     // If you only want to retrieve the first matching element, you can break the loop here
//     }
//   }
// });
...
$(document).ready(function () {
  $(".dropdown-item").click(function () {
    ...
    $.ajax({
      ...
      //   url: base_url + "helpers/function_helper/save_branch",
...
// select multiple ids of branch while adding product

// $(document).ready(function () {
//   $("#mySelect").on("change", function () {
//     var selectedOptionIds = $(this).val();
//     console.log(selectedOptionIds); // Display selected option IDs in the console

// Send the selectedOptionIds to a PHP script using AJAX if required

/*
      $.ajax({
        url: 'process.php',
        type: 'POST',
        data: { selectedOptions: selectedOptionIds },
        success: function(response) {
