...
class Ion_auth
{
    ...
    {
        return $this->ion_auth_model->in_group('partner', $id);
    }
    public function partner_status($id = FALSE)
    {
        $id = ($id == FALSE) ? $this->session->userdata('user_id') : $id;
        if (!empty($id) && $id != FALSE) {
            $status = fetch_details(['user_id' => $id], 'partner_data', 'status');
    ...
    public function is_rider($id = FALSE)
    {
        return $this->ion_auth_model->in_group('rider', $id);
    }

    public function is_waiter($id = FALSE)
    {
        return $this->ion_auth_model->in_group('waiter', $id);
    }
