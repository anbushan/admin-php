...
function fetch_product($branch_id = NULL, $user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $sort_by = NULL)
{
    ...
    // if (isset($filter) && !empty($filter['city_id']) && $filter['city_id'] != "" && !empty($filter['latitude']) && !empty($filter['longitude'])) {
    //     $where['u.city'] = $filter['city_id'];
    //     $where_near_by = "ST_Distance_Sphere(POINT(u.latitude,u.longitude), ST_GeomFromText('POINT(" . $filter['latitude'] . " " . $filter['longitude'] . ")') )/ 1000 <=  " . $near_by_distance;
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]['relative_path'] =  base_url() . $relative_path;
            ...
            $product[$i]['category_slug'] =  isset($product[$i]['category_slug']) && !empty($product[$i]['category_slug']) ? output_escaping($product[$i]['category_slug']) : ''; //zipcode123
            ...
            if (!empty($product[$i]['variants'])) {
                ...
                for ($k = 0; $k < count($product[$i]['variants']); $k++) {
                    ...
                    if (isset($user_id) && $user_id != NULL) {
                        ...
                        // print_r($current_branch_id);
                        // die;
                        $user_cart_data = get_cart_total($user_id, $product[$i]['variants'][$k]['id'], '', '', $current_branch_id);
                        // print_r($user_cart_data);
    ...
    // if ($partner_id != NULL) {
    //     $category_list = $t->db->select('id', 'name')->where("id in(select category_id from products where partner_id = $partner_id)")->get('categories')->result_array();
    // }
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '', $branch_id = NULL)
{
    ...
    // print_r(gettype($is_saved_for_later));
    // die;
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $download_thermal_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $branch_id = null, $is_pending = false, $rider_city_id = NULL, $rejected_rider = NULL)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,p.cancelable_till,p.is_returnable,p.image,p.name,p.indicator,p.type,(Select count(id)
 from order_items where order_id = oi.order_id ) as order_counter')
        ...
        if ((isset($rider_id) && $rider_id != null)) {
            ...
            // $order_details[$i]['total'] = strval($order_details[$i]['total'] +  intval($total_tax_amount));
            $order_details[$i]['total'] = strval($order_details[$i]['total']);
        }

        // print_r($order_details[$i]['username']);
        ...
        if ($download_thermal_invoice == true || $download_thermal_invoice == 1) {
            $order_details[$i]['thermal_invoice_html'] =  get_thermal_invoice_html($order_details[$i]['id']);
        }
        if (!empty($order_item_data)) {
...
function get_favorites($user_id, $branch_id, $type = 'products', $limit = NULL, $offset = NULL)
{
    ...
    if ($type == 'products') {
        ...
        $q = $CI->db->join('products p', 'p.id=f.type_id')
            ...
            ->get('favorites f');
        $res = $q->result_array();
        // $total_products = $CI->db->select("(select count(id) from favorites where user_id= $user_id and type='products')")->get('favorites f')->result_array();
        $where = [
            'f.user_id' => $user_id,
            'f.type' => $type
        ];
        ...
        //  (SELECT MIN(if(pv.special_price != 0, pv.special_price, pv.price)) FROM `partner_data` pd JOIN products p on p.partner_id = pd.user_id JOIN product_variants pv ON pv.product_id=p.id WHERE pd.user_id=sd.user_id ) as price_for_one')
...
function verify_payment_transaction($txn_id, $payment_method, $additional_data = [])
{
    ...
    switch ($payment_method) {
        ...
        case 'paypal':
            ...
            //     $transaction = $CI->midtrans->get_transaction_status($txn_id);
...
//         $partner_data = fetch_details(['id' => $partner_id], "users", "latitude,longitude");
//         $where = "ST_Distance_Sphere(POINT(latitude,longitude), ST_GeomFromText('POINT(" . $partner_data[0]['latitude'] . " " . $partner_data[0]['longitude'] . ")') ) as distance";
//         // here we are getting partner distance and points
//         $data = fetch_details(['id' => $partner_id], "users", "latitude,longitude,$where,city");
//         if ($type == "city") {
//             $partner = fetch_details(['id' => $address_id], 'cities', 'geolocation_type,radius,boundary_points,max_deliverable_distance');
//         } else {
//             $partner = fetch_details(['id' => $data[0]['city']], 'cities', 'geolocation_type,radius,boundary_points,max_deliverable_distance');
//         }
...
// function send_pending_order_notification($role_id = "", $role = "rider", $title = "", $body = "", $type = "", $id = "")
// {
//     $t = &get_instance();
//     $settings = get_settings('system_settings', true);
//     $fcm_ids = array();
