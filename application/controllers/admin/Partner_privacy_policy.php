...
class Partner_privacy_policy extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->data['main_page'] = FORMS . 'partner-privacy-policy';
            $settings = get_settings('system_settings', true);
            $this->data['title'] = 'Partner Privacy Policy | ' . $settings['app_name'];
            $this->data['meta_description'] = 'Partner Privacy Policy | ' . $settings['app_name'];
            $this->data['privacy_policy'] = get_settings('partner_privacy_policy');
            $this->data['terms_n_condition'] = get_settings('partner_terms_conditions');
    ...
    public function update_privacy_policy_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->Setting_model->update_partner_privacy_policy($_POST);
                $this->Setting_model->update_partner_terms_n_condtions($_POST);
    ...
    public function terms_and_conditions_page()
    {
        ...
        $this->data['terms_and_conditions'] = get_settings('partner_terms_conditions');
