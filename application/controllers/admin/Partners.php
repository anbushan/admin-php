...
class Partners extends CI_Controller
{
    ...
    public function view_citywise_partners()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $search =  (isset($_GET['search'])) ? $_GET['search'] : null;
            $partners =  $this->Partner_model->get_citywise_partners_list();
            // print_R($partners);
            $this->response['data'] = $partners;
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($licence_files['licence_proof']) && !empty($licence_files['licence_proof']['name'][0]) && isset($licence_files['licence_proof']['name'][0])) {
                    ...
                    // if (isset($_POST['edit_restro']) && !empty($_POST['edit_restro']) && isset($_POST['old_licence_proof']) && !empty($_POST['old_licence_proof'])) {
                    //     $old_licence_proof =  json_encode($this->input->post('old_licence_proof', true));
                    //     delete_images(RESTRO_DOCUMENTS_PATH, $old_licence_proof);
                    // }
                    // print_R($old_licence_proof);
                    // return;
                ...
                if (isset($_POST['edit_restro'])) {
                    $old_licence = fetch_details(['user_id' => $_POST['edit_restro']], 'partner_data', 'licence_proof');
                    // if (!edit_unique($this->input->post('email', true), 'users.email.' . $this->input->post('edit_restro', true) . '') || !edit_unique($this->input->post('mobile', true), 'users.mobile.' . $this->input->post('edit_restro', true) . '')) {
                    //     $response["error"]   = true;
