...
class Rating_model extends CI_Model
{
    ...
    public function set_rider_rating($data)
    {
        ...
        if ($this->db->where(['user_id' => $data['user_id'], 'rider_id' => $data['rider_id']])->get('rider_rating')->num_rows() > 0) {
            // $this->db->where(['user_id' => $data['user_id'], 'rider_id' => $data['rider_id']])->update('rider_rating', $rating);
    ...
    public function delete_rating($rating_id, $order_rating_id = '')
    {
        ...
        if (isset($rating_id) && !empty($rating_id)) {
            ...
            $product_id = $rating_details[0]['product_id'];

            $product_rating = $this->db->select('rating, (select count(rating) from product_rating where product_id=' . $product_id . ') as no_of_ratings, (select sum(rating) from product_rating where product_id=' . $product_id . ') as sum_of_rating')->where('id', $product_id)->get('products')->result_array();
            ...
            $this->db->set(['rating' => $newrating, 'no_of_ratings' => $no_of_rating])->where('id', $product_id)->update('products');

            $branch_id = fetch_details(['id' => $product_id], 'products', 'branch_id');

            $branch_id = $branch_id[0]['branch_id'];
            $where = "branch_id = $branch_id and rating > 0";
    ...
    function partner_id($product_id = "", $order_id = "", $user_id = "", $limit = "", $offset = "", $sort = "", $order = "", $rating_id = null, $has_images = null)
    {
