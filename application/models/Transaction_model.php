...
class Transaction_model extends CI_Model
{
    ...
    function get_transactions_list($user_id = '', $group_id = 2)
    {
        ...
        if (isset($group_id[0]['id']) && !empty($group_id[0]['id'])) {
        ...
        foreach ($txn_search_res as $row) {
            ...
            if (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'success') {

                $tempRow['status'] = '<span class="badge badge-success">Success</span>';
            } elseif (isset($row['status']) && !empty($row['status']) && !is_null($row['status']) && $row['status'] == 'faield') {
                $tempRow['status'] = '<span class="badge badge-danger">Faield</span>';
            } else {
