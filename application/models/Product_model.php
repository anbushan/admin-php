...
class Product_model extends CI_Model
{
    ...
    public function add_product($data, $tags = [])
    {
        ...
        $pro_data = [
            ...
            // 'branch_ids' => implode(",", $data['branch']),
            'indicator' => $indicator,
        ...
        //         for ($x = 0; $x < count($last_product_id); $x++) {
        //             $add_ons['product_id'] = $last_product_id[$x];
    ...
    public function get_product_details($flag = NULL,  $p_status = NULL)
    {
        ...
        // print_r($_SESSION);
        // die;
        // $count_res->where('p.branch_id', $_SESSION['branch_id']);
        ...
        $search_res = $this->db->select('product_variants.id AS id, p.id as pid ,p.rating,p.no_of_ratings,p.name,p.branch_id as branch_id ,p.type, p.image, p.status,product_variants.price , product_variants.special_price, product_variants.stock ')
